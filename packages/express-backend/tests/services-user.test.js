...
describe('getUsers', () => {
    ...
    it('should handle errors during fetching users', async () => {
        const findMock = jest.fn().mockRejectedValue(new Error('Error fetching users'));
        userModel.find = findMock;
...
describe('getUserByNameAndPassword', () => {
    it('should return a user when username and password are provided', async () => {
        ...
        const result = await userService.getUserByNameAndPassword(username, password);
    ...
    it('should handle errors during fetching user', async () => {
        const username = 'testuser';
        const password = 'testpass';
        const findOneMock = jest.fn().mockRejectedValue(new Error('Error fetching user'));
        userModel.findOne = findOneMock;

        await expect(userService.getUserByNameAndPassword(username, password)).rejects.toThrow('Error fetching user');
    });
});
...
describe('addUser', () => {
    ...
    it('should handle errors during adding user', async () => {
        const user = { username: 'testuser', password: 'testpass' };
        const saveMock = jest.fn().mockRejectedValue(new Error('Error adding user'));

        userModel.mockImplementation(() => ({
            save: saveMock,
        }));

        await expect(userService.addUser(user)).rejects.toThrow('Error adding user');
    });
});
...
describe('editUser', () => {
    it('should update a user by id', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockResolvedValue(updatedUserData);
        userModel.findByIdAndUpdate = findByIdAndUpdateMock;

        const result = await userService.editUser(userId, updatedUser);

        expect(findByIdAndUpdateMock).toHaveBeenCalledWith(userId, updatedUser, { new: true });
    ...
    it('should handle errors during updating user', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockRejectedValue(new Error('Error updating user'));
        userModel.findByIdAndUpdate = findByIdAndUpdateMock;

        await expect(userService.editUser(userId, updatedUser)).rejects.toThrow('Error updating user');
    });
});
...
describe('findUserById', () => {
    ...
    it('should handle errors during fetching user', async () => {
        ...
        const findByIdMock = jest.fn().mockRejectedValue(new Error('Error fetching user'));
        userModel.findById = findByIdMock;

        await expect(userService.findUserById(userId)).rejects.toThrow('Error fetching user');
    });
});
...
describe('deleteUserById', () => {
    ...
    it('should handle errors during deleting user', async () => {
        ...
        const findByIdAndDeleteMock = jest.fn().mockRejectedValue(new Error('Error deleting user'));
        userModel.findByIdAndDelete = findByIdAndDeleteMock;

        await expect(userService.deleteUserById(userId)).rejects.toThrow('Error deleting user');
    });
});