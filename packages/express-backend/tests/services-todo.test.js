...
describe('getTodoItems', () => {
    it('should return todo items for a given due date and userId', async () => {
        ...
        const userId = 'userId';
        const todoItems = [{ duedate: duedate, user: userId, task: 'Task1' }, { duedate: duedate, user: userId, task: 'Task2' }];
        ...
        expect(findMock).toHaveBeenCalledWith({ duedate: duedate, user: userId });
        expect(result).toEqual(todoItems);
    });

    it('should handle errors during fetching todo items', async () => {
        const findMock = jest.fn().mockRejectedValue(new Error('Error fetching todo items'));
        todoModel.find = findMock;

        await expect(todoService.getTodoItems()).rejects.toThrow('Error fetching todo items');
    });
});
...
describe('addTodoItem', () => {
    ...
    it('should handle errors during adding todo item', async () => {
        const item = { task: 'Task1' };
        const saveMock = jest.fn().mockRejectedValue(new Error('Error adding todo item'));

        todoModel.mockImplementation(() => ({
            save: saveMock,
        }));

        await expect(todoService.addTodoItem(item)).rejects.toThrow('Error adding todo item');
    });
});
...
describe('editTodoItem', () => {
    it('should update a todo item by id', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockResolvedValue(updatedItemData);
        todoModel.findByIdAndUpdate = findByIdAndUpdateMock;

        const result = await todoService.editTodoItem(itemId, updatedItem);

        expect(findByIdAndUpdateMock).toHaveBeenCalledWith(itemId, updatedItem, { new: true });
    ...
    it('should handle errors during updating todo item', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockRejectedValue(new Error('Error updating todo item'));
        todoModel.findByIdAndUpdate = findByIdAndUpdateMock;

        await expect(todoService.editTodoItem(itemId, updatedItem)).rejects.toThrow('Error updating todo item');
    });
});
...
describe('findTodoItemById', () => {
    ...
    it('should handle errors during fetching todo item', async () => {
        ...
        const findByIdMock = jest.fn().mockRejectedValue(new Error('Error fetching todo item'));
        todoModel.findById = findByIdMock;

        await expect(todoService.findTodoItemById(itemId)).rejects.toThrow('Error fetching todo item');
    });
});
...
describe('deleteTodoItemById', () => {
    ...
    it('should handle errors during deleting todo item', async () => {
        ...
        const findByIdAndDeleteMock = jest.fn().mockRejectedValue(new Error('Error deleting todo item'));
        todoModel.findByIdAndDelete = findByIdAndDeleteMock;

        await expect(todoService.deleteTodoItemById(itemId)).rejects.toThrow('Error deleting todo item');
    });
});
