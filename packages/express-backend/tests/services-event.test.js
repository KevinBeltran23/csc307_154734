...
describe('getEvents', () => {
    it('should return events based on start, calendar, and userId', async () => {
        ...
        expect(findMock).toHaveBeenCalledWith({ start, calendar, user: userId });
    ...
    it('should return events based on userId', async () => {
        ...
        const result = await eventService.getEvents(undefined, undefined, userId);
    ...
    it('should handle errors during fetching events', async () => {
        const findMock = jest.fn().mockRejectedValue(new Error('Error fetching events'));
        eventModel.find = findMock;

        await expect(eventService.getEvents()).rejects.toThrow('Error fetching events');
    });
});

describe('addEvent', () => {
    it('should add a new event', async () => {
        const event = { name: 'event1', start: new Date(), calendar: 'calendarId', user: 'userId' };
    ...
    it('should handle errors during adding event', async () => {
        const event = { name: 'event1', start: new Date(), calendar: 'calendarId', user: 'userId' };
        const saveMock = jest.fn().mockRejectedValue(new Error('Error adding event'));

        eventModel.mockImplementation(() => ({
            save: saveMock,
        }));

        await expect(eventService.addEvent(event)).rejects.toThrow('Error adding event');
    });
});
...
describe('editEvent', () => {
    it('should update an event by id', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockResolvedValue(updatedEventData);
        eventModel.findByIdAndUpdate = findByIdAndUpdateMock;

        const result = await eventService.editEvent(eventId, updatedEvent);

        expect(findByIdAndUpdateMock).toHaveBeenCalledWith(eventId, updatedEvent, { new: true });
    ...
    it('should handle errors during updating event', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockRejectedValue(new Error('Error updating event'));
        eventModel.findByIdAndUpdate = findByIdAndUpdateMock;

        await expect(eventService.editEvent(eventId, updatedEvent)).rejects.toThrow('Error updating event');
    });
});
...
describe('findEventById', () => {
    ...
    it('should handle errors during fetching event', async () => {
        ...
        const findByIdMock = jest.fn().mockRejectedValue(new Error('Error fetching event'));
        eventModel.findById = findByIdMock;

        await expect(eventService.findEventById(eventId)).rejects.toThrow('Error fetching event');
    });
});
...
describe('deleteEventById', () => {
    ...
    it('should handle errors during deleting event', async () => {
        ...
        const findByIdAndDeleteMock = jest.fn().mockRejectedValue(new Error('Error deleting event'));
        eventModel.findByIdAndDelete = findByIdAndDeleteMock;

        await expect(eventService.deleteEventById(eventId)).rejects.toThrow('Error deleting event');
    });
});
