...
describe('getClasses', () => {
    it('should return classes for given start, calendar, and userId', async () => {
        ...
        const userId = 'userId';
        const classes = [{ start, calendar, user: userId, name: 'Class1' }, { start, calendar, user: userId, name: 'Class2' }];
    ...
    it('should handle errors during fetching classes', async () => {
        const findMock = jest.fn().mockRejectedValue(new Error('Error fetching classes'));
        classModel.find = findMock;

        await expect(classService.getClasses()).rejects.toThrow('Error fetching classes');
    });
});
...
describe('addClass', () => {
    ...
    it('should handle errors during adding class', async () => {
        const event = { name: 'Class1' };
        const saveMock = jest.fn().mockRejectedValue(new Error('Error adding class'));

        classModel.mockImplementation(() => ({
            save: saveMock,
        }));

        await expect(classService.addClass(event)).rejects.toThrow('Error adding class');
    });
});
...
describe('editClass', () => {
    it('should update a class by id', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockResolvedValue(updatedClassData);
        classModel.findByIdAndUpdate = findByIdAndUpdateMock;

        const result = await classService.editClass(classId, updatedClass);

        expect(findByIdAndUpdateMock).toHaveBeenCalledWith(classId, updatedClass, { new: true });
    ...
    it('should handle errors during updating class', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockRejectedValue(new Error('Error updating class'));
        classModel.findByIdAndUpdate = findByIdAndUpdateMock;

        await expect(classService.editClass(classId, updatedClass)).rejects.toThrow('Error updating class');
    });
});
...
describe('findClassById', () => {
    ...
    it('should handle errors during fetching class', async () => {
        ...
        const findByIdMock = jest.fn().mockRejectedValue(new Error('Error fetching class'));
        classModel.findById = findByIdMock;

        await expect(classService.findClassById(classId)).rejects.toThrow('Error fetching class');
    });
});
...
describe('deleteClassById', () => {
    ...
    it('should handle errors during deleting class', async () => {
        ...
        const findByIdAndDeleteMock = jest.fn().mockRejectedValue(new Error('Error deleting class'));
        classModel.findByIdAndDelete = findByIdAndDeleteMock;

        await expect(classService.deleteClassById(classId)).rejects.toThrow('Error deleting class');
    });
});
