...
describe('getCalendars', () => {
    it('should return calendars for a given userId', async () => {
        const userId = 'userId';
        const calendars = [{ user: userId, name: 'Calendar1' }, { user: userId, name: 'Calendar2' }];
    ...
    it('should handle errors during fetching calendars', async () => {
        const findMock = jest.fn().mockRejectedValue(new Error('Error fetching calendars'));
        calendarModel.find = findMock;

        await expect(calendarService.getCalendars()).rejects.toThrow('Error fetching calendars');
    });
});
...
describe('addCalendar', () => {
    ...
    it('should handle errors during adding calendar', async () => {
        const calendar = { name: 'Calendar1' };
        const saveMock = jest.fn().mockRejectedValue(new Error('Error adding calendar'));

        calendarModel.mockImplementation(() => ({
            save: saveMock,
        }));

        await expect(calendarService.addCalendar(calendar)).rejects.toThrow('Error adding calendar');
    });
});
...
describe('editCalendar', () => {
    it('should update a calendar by id', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockResolvedValue(updatedCalendarData);
        calendarModel.findByIdAndUpdate = findByIdAndUpdateMock;

        const result = await calendarService.editCalendar(calendarId, updatedCalendar);

        expect(findByIdAndUpdateMock).toHaveBeenCalledWith(calendarId, updatedCalendar, { new: true });
    ...
    it('should handle errors during updating calendar', async () => {
        ...
        const findByIdAndUpdateMock = jest.fn().mockRejectedValue(new Error('Error updating calendar'));
        calendarModel.findByIdAndUpdate = findByIdAndUpdateMock;

        await expect(calendarService.editCalendar(calendarId, updatedCalendar)).rejects.toThrow('Error updating calendar');
    });
});
...
describe('findCalendarById', () => {
    ...
    it('should handle errors during fetching calendar', async () => {
        ...
        const findByIdMock = jest.fn().mockRejectedValue(new Error('Error fetching calendar'));
        calendarModel.findById = findByIdMock;

        await expect(calendarService.findCalendarById(calendarId)).rejects.toThrow('Error fetching calendar');
    });
});
...
describe('deleteCalendarById', () => {
    it('should delete a calendar by id', async () => {
        ...
        const findByIdAndDeleteMock = jest.fn().mockResolvedValue(deletedCalendar);
    ...
    it('should handle errors during deleting calendar', async () => {
        ...
        const findByIdAndDeleteMock = jest.fn().mockRejectedValue(new Error('Error deleting calendar'));
        calendarModel.findByIdAndDelete = findByIdAndDeleteMock;

        await expect(calendarService.deleteCalendarById(calendarId)).rejects.toThrow('Error deleting calendar');
    });
});
